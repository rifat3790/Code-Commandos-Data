...
export default function Dashboard({ csvData }: { csvData: string }) {
  ...
  const getStatusColor = (status: string) => {
    ...
    if (s.includes('wip')) return 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30';
    if (s.includes('cancel')) return 'bg-rose-500/20 text-rose-400 border-rose-500/30';
    if (s.includes('done') || s.includes('complete') || s.includes('delivered')) return 'bg-blue-500/20 text-blue-400 border-blue-500/30';
    return 'bg-slate-500/20 text-slate-400 border-slate-500/30';
  };

  const getTimelineStyle = (timeline: string) => {
    const t = (timeline || "").toLowerCase();
    if (t.includes('order late')) return 'bg-rose-600 text-white border border-rose-500 shadow-lg shadow-rose-600/30 px-3 py-1 rounded-lg font-bold tracking-wide text-xs uppercase animate-pulse';
    if (t.includes('cancel') || t.includes('done') || t.includes('delivered')) return 'text-slate-400 font-bold';
    if (t.includes('s')) {
      const hasDays = t.includes('d');
      if (!hasDays || t.match(/^0d/)) return 'text-rose-500 font-extrabold';
      const dMatch = t.match(/(\d+)d/);
      if (dMatch && parseInt(dMatch[1]) <= 2) return 'text-amber-400 font-bold';
      return 'text-emerald-400 font-medium';
    }
    if (t.includes('day')) {
      const days = parseInt(t);
      if (days <= 2) return 'text-amber-400 font-bold';
      return 'text-emerald-400 font-medium';
    }
      ...
      <div className="flex flex-col md:flex-row gap-6 justify-between items-start md:items-center">
        <div className="flex flex-wrap gap-4">
          <div className="bg-slate-800/50 backdrop-blur-md border border-slate-700/50 rounded-2xl p-4 flex items-center gap-4 shadow-lg">
            ...
            <div>
              <p className="text-sm text-slate-400 font-medium">Filtered Records</p>
          ...
          <div className="bg-slate-800/50 backdrop-blur-md border border-slate-700/50 rounded-2xl p-4 flex items-center gap-4 shadow-lg">
            ...
            <div>
              <p className="text-sm text-slate-400 font-medium">Active (WIP)</p>
          ...
          <div className="bg-slate-800/50 backdrop-blur-md border border-slate-700/50 rounded-2xl p-4 flex items-center gap-4 shadow-lg">
            ...
            <div>
              <p className="text-sm text-slate-400 font-medium">Total Value</p>
              <p className="text-2xl font-bold text-white">
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-800/90 text-slate-300 font-medium border-b border-slate-700/50">
              <tr>
                ...
                <th className="px-5 py-4 tracking-wide sticky right-0 bg-slate-800/90 shadow-[-10px_0_15px_-3px_rgba(0,0,0,0.3)] border-l border-slate-700/50 text-indigo-300">Live Timeline</th>
              </tr>
            </thead>
                    ...
                    >
                      {allColumns.filter(c => visibleColumns.includes(c)).map(col => {
                        ...
                        if (col.toLowerCase() === 'order id') {
                           return (
                             <td key={col} className="px-5 py-3 font-mono text-slate-400 text-xs">
                               {val}
                             </td>
                           );
                        }
                      ...
                      <td className="px-5 py-3 sticky right-0 bg-slate-900/90 group-hover:bg-slate-800/90 shadow-[-10px_0_15px_-3px_rgba(0,0,0,0.3)] border-l border-slate-700/50 transition-colors">
